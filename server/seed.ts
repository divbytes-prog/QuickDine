...
const seedData = async () => {
    try {
        ...
        const ownerPassword = await bcrypt.hash("owner123", salt) // NOTE: screenshot showed "wner123" — cut off on the left edge, almost certainly "owner123"
        ...
        const testUser = await User.create({
            ...
            phone: "+01234567788",
            role: "user", // NOTE: this line was overlapped by the video's progress bar in the screenshot and showed "admin" — "user" is almost certainly correct given the comment and variable name
        })
        ...
        const restaurantsData: SeedRestaurant[] = [
            {
                ...
                featured: true,
                exclusive: false,
                // owner / status / totalSeats / createdAt / updatedAt not visible in any screenshot for this entry
            },
            {
        ...
        // Not visible in any screenshot — added so restaurantsData actually gets saved.
        // Uses ownerUser._id for any restaurant that didn't already specify an owner.
        for (const restaurant of restaurantsData) {
            await Restaurant.create({
                ...restaurant,
                owner: restaurant.owner || ownerUser._id,
            });
        }
        console.log(`Created ${restaurantsData.length} restaurants.`);

        // Booking seed data wasn't visible in any screenshot, so it isn't included here.
